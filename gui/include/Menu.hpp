...
#define MAX_INPUT_CHARS     9
    ...
    public:
        ...
        void LoadResources(Model &model, Texture2D &texture_body, Texture2D &texture_leaf, Texture2D &gemsTexture);
        void ConfigureCamera(Camera &camera);
        void MainLoop(Model model, Texture2D background, Camera camera, Vector3 position, BoundingBox bounds, Zappy::Draw &draw, Texture2D gemsTexture);
        void UnloadResources(Model model, Texture2D texture_body, Texture2D texture_leaf, Texture2D gemsTexture);
        void LoopForTextbox(Rectangle textBox, bool &mouseOnText, char name[MAX_INPUT_CHARS + 1], int &letterCount, int &framesCounter);
        void GameScene(Model model, Camera camera, Vector3 position, BoundingBox bounds, Texture2D gemsTexture);
